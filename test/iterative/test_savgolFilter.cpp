...
#ifdef FLOAT_DOUBLE
    -0.0219, -0.0156, -0.0099, -0.0047, 0.0,
    0.0035, 0.0065, 0.0090, 0.0111, 0.0127,
    0.0139, 0.0146, 0.0149, 0.0146, 0.0139,
    0.0127, 0.0111, 0.0090, 0.0065, 0.0035,
    0.0, -0.0047, -0.0099, -0.0156, -0.0219};
#elif FLOAT_LONG_DOUBLE
    -0.0219l, -0.0156l, -0.0099l, -0.0047l, 0.0l,
    0.0035l, 0.0065l, 0.0090l, 0.0111l, 0.0127l,
    0.0139l, 0.0146l, 0.0149l, 0.0146l, 0.0139l,
    0.0127l, 0.0111l, 0.0090l, 0.0065l, 0.0035l,
    0.0l, -0.0047l, -0.0099l, -0.0156l, -0.0219l};
#else
...
#ifdef FLOAT_DOUBLE
    0.2143, 0.1429, 0.0781, 0.0195, -0.0336,
    -0.0812, -0.1240, -0.1623, -0.1964, -0.2266,
    -0.2532, -0.2766, -0.2970, -0.3147, -0.3300,
    -0.3430, -0.3540, -0.3632, -0.3708, -0.3769,
    -0.3817, -0.3853, -0.3879, -0.3895, -0.3903};
#elif FLOAT_LONG_DOUBLE
    0.2143l, 0.1429l, 0.0781l, 0.0195l, -0.0336l,
    -0.0812l, -0.1240l, -0.1623l, -0.1964l, -0.2266l,
    -0.2532l, -0.2766l, -0.2970l, -0.3147l, -0.3300l,
    -0.3430l, -0.3540l, -0.3632l, -0.3708l, -0.3769l,
    -0.3817l, -0.3853l, -0.3879l, -0.3895l, -0.3903l};
#else
...
#ifdef FLOAT_DOUBLE
    -0.3903, -0.3895, -0.3879, -0.3853, -0.3817,
    -0.3769, -0.3708, -0.3632, -0.3540, -0.3430,
    -0.3300, -0.3147, -0.2970, -0.2766, -0.2532,
    -0.2266, -0.1964, -0.1623, -0.1240, -0.0812,
    -0.0336, 0.0195, 0.0781, 0.1429, 0.2143};
#elif FLOAT_LONG_DOUBLE
    -0.3903l, -0.3895l, -0.3879l, -0.3853l, -0.3817l,
    -0.3769l, -0.3708l, -0.3632l, -0.3540l, -0.3430l,
    -0.3300l, -0.3147l, -0.2970l, -0.2766l, -0.2532l,
    -0.2266l, -0.1964l, -0.1623l, -0.1240l, -0.0812l,
    -0.0336l, 0.0195l, 0.0781l, 0.1429l, 0.2143l};
#else
...
TEST_F(SavitzkyGolayFilterTest, TrailingEdge)
{
    ...
    for (int i = (int)dataSize - halfWindowSize; i < static_cast<int>(dataSize); ++i)
    {
        EXPECT_TRUE(isClose(filteredData[i].phaseAngle, expectedOutput[i], 1e-5))
...
TEST_F(SavitzkyGolayFilterTest, CenterCase)
{
    ...
    int centerStart = (int)dataSize / 2 - 5;
    int centerEnd = (int)dataSize / 2 + 5;
...
TEST_F(SavitzkyGolayFilterTest, MemoizationWorks)
{
...
#ifdef _MSC_VER
    MqsRawDataPoint_t* smallRawData = (MqsRawDataPoint_t*)malloc(windowSize*sizeof(MqsRawDataPoint_t));
    MqsRawDataPoint_t* smallFilteredData = (MqsRawDataPoint_t*)malloc(windowSize*sizeof(MqsRawDataPoint_t));
#else
    ...
    {
        smallRawData[i].phaseAngle = ONE;
        smallFilteredData[i].phaseAngle = ZERO;
    }
...
TEST_F(SavitzkyGolayFilterTest, MemoizationCacheBoundariesAndConsistency)
{
    ...
    for (size_t i = 0; i < smallDataSize; ++i)
    {
        smallRawData[i].phaseAngle = TWO; // Different dataset
        smallFilteredData[i].phaseAngle = ZERO;
    }
    GramPolyIterative_fake.call_count = 0;
    result = mes_savgolFilter(smallRawData, smallDataSize, halfWindowSize, smallFilteredData, polynomialOrder, targetPoint, derivativeOrder);
...
TEST_F(SavitzkyGolayFilterTest, DerivativeOrders)
{
    ...
    for (size_t i = 0; i < smallDataSize; ++i)
    {
        smallRawData[i].phaseAngle = (FLOAT)(i * i);
        smallFilteredData[i].phaseAngle = ZERO;
    }
    ...
    FLOAT expectedFirstDerivative[] = {ZERO, TWO, FOUR, SIX, EIGHT, TEN, TWELVE};
    ...
    FLOAT expectedSecondDerivative[] = {TWO, TWO, TWO, TWO, TWO, TWO, TWO};
    for (size_t i = halfWindowSize; i < smallDataSize - halfWindowSize; ++i)
    {
...
TEST_F(SavitzkyGolayFilterTest, DifferentWindowSizesAndOrders)
{
    ...
    for (int i = 0; i < 41; ++i)
    {
        mockWeightsLarge[i] = ONE / FORTYONE;
    }
    ComputeWeights_fake.custom_fake = [](uint8_t hws, uint16_t tp, uint8_t po, uint8_t d, FLOAT *weights)
    {
        for (int i = 0; i < 41; ++i)
        {
            weights[i] = ONE / FORTYONE;
        }
    };
    for (int i = halfWindowSize; i < smallDataSize - halfWindowSize; i += 10)
    {
...
TEST_F(SavitzkyGolayFilterTest, NumericalStability)
{
    ...
    {
        noiseRawData[i].phaseAngle = (i % 2 == 0) ? ONE : -ONE;
        noiseFilteredData[i].phaseAngle = ZERO;
    }
    ...
    {
        EXPECT_TRUE(fabs(noiseFilteredData[i].phaseAngle) < HALV)
    ...
    {
        flatRawData[i].phaseAngle = FIVE;
        flatFilteredData[i].phaseAngle = ZERO;
    }
    ...
    for (size_t i = 0; i < flatDataSize; ++i)
    {
        EXPECT_TRUE(isClose(flatFilteredData[i].phaseAngle, FIVE, 1e-5))
